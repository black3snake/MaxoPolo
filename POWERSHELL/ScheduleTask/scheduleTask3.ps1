...
Add-Content -Path $sched_output -Value "ServerName;TaskName; State; Author; RunAs"
...
foreach($ComputerName in $ServDCOM) {
    ...
    try {
        ...
        $ER_catch = $Error[0].Exception.Message
        Write-Host $ER_catch -ForegroundColor Red
        Add-Content -Path $sched_output -Value "Ошибка в создании CIM сессии для $ComputerName : $ER_catch"
        continue
    }
    ...
    try {
        ...
        foreach($item in $REQ22) {
	        ...
	        Add-Content -Path $sched_output -Value "$ComputerName; $Name ; $State ; $Author ; $CID"
		        
        }
        Get-CimSession | Where-Object {$_.ComputerName -match $ComputerName} | Remove-CimSession

	} catch {
        $ER_catch2 = $Error[0].Exception.Message
        Write-Host $ER_catch2 -ForegroundColor Red
        Add-Content -Path $sched_output -Value "$ER_catch2"
        continue
    }
    
    #} else {
        #    Add-Content -Path $sched_output -Value "Ошибка в доступе к серверу (нет пинга)"
    #}
